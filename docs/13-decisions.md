...
**Решение:** Кнопка `Назад в лобби` и Telegram Back со стола не выполняют скрытый leave. Они сворачивают стол в лобби после подтверждения и явно объясняют, что игрок остаётся за столом. Реальный выход со стола вынесен в отдельное действие `Покинуть стол` с подтверждением и возвратом стека по правилам. Если игрок уже сидит за столом, клик по этому столу в лобби открывает стол напрямую без повторного buy-in sheet. `Отойти` становится одношаговым действием: если рука активна, сервер ставит sit-out со следующей раздачи, а UI показывает это состояние.
...
**Что обновлено:** Разделены lobby navigation и table leave, добавлено подтверждение, прямой reopen занятого стола, cash buy-in отображается в USDT без `$` и без внутренних micros/chips.
