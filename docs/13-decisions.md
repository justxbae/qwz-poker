...
**Решение:** Кнопка `Назад в лобби` и Telegram Back со стола не выполняют скрытый leave. Они сворачивают стол в лобби после короткого подтверждения и игрок остаётся за столом. Реальный выход со стола вынесен в отдельное действие `Покинуть стол` с коротким подтверждением и возвратом стека по правилам. Leave/stand используют request-level idempotency key, а не повторяемый ключ из `table/user/hand/stack`, чтобы новый выход не мог поднять старый snapshot. Если игрок уже сидит за столом, клик по этому столу в лобби открывает стол напрямую без повторного buy-in sheet. `Отойти` становится одношаговым действием: если рука активна, сервер ставит sit-out со следующей раздачи, а UI показывает это состояние. Snapshot-события при открытии стола только синхронизируют realtime cursor и не проигрываются пачкой как новые анимации. В Telegram Mini App отключаются vertical swipes через WebApp API, а системные scrollbars скрываются CSS.
**Почему:** Back navigation и leave были смешаны, из-за чего игрок мог оказаться в лобби без понятного статуса и при повторном входе получать buy-in sheet, хотя уже сидит за столом.
**Влияет на:** `public/app.js`, `public/index.html`, in-game side menu, lobby table list, buy-in sheet.
**Что обновлено:** Разделены lobby navigation и table leave, добавлено подтверждение, прямой reopen занятого стола, cash buy-in отображается в USDT без `$` и без внутренних micros/chips, исправлен stale snapshot/idempotency replay на leave, подавлен replay старых table events, отключён Telegram vertical swipe collapse и скрыты scrollbars.
**Открытые вопросы:** Для публичного релиза можно заменить native confirm на фирменный bottom-sheet confirm, если понадобится более брендированный UX.
